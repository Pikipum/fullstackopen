...
describe('test regarding blogs', () => {
    beforeEach(async () => {
        ...
        const user = new User({ _id: userId, username: 'JohnUser1337', passwordHash, name: 'John Tester' })
        await user.save()
    ...
    test('blogs can be added', async () => {
        ...
        await api.post('/api/blogs').set('Authorization', `Bearer ${loggedUser.body.token}`).send(helper.testBlog)
    ...
    test('updating a blog by id returns 200 OK', async () => {
        const loggedUser = await api.post('/api/login').send(helper.testUser)
        await api.put(`/api/blogs/${helper.initialBlogs[2]._id}`)
            .set('Authorization', `Bearer ${loggedUser.body.token}`)
